...
for(const group of groups)console.log((await workbook.inspect({ kind: "table", range: `${group.name}!A1:O20`, include: "values,formulas", tableMaxRows: 20, tableMaxCols: 15 })).ndjson);
...
function buildInvoices(s,invoices,sheetLabel) {
  ...
  s.getRange("A1:O1").merge(); s.getRange("A1").values=[[`REPORTE DE ${sheetLabel.toUpperCase()} · COMPROBANTES ELECTRÓNICOS`]];
  s.getRange("A1:O1").format={fill:green,font:{bold:true,color:"#FFFFFF",size:16},rowHeight:32,verticalAlignment:"center"};
  s.getRange("A2:O2").values=[["Razón social",data.metadata.businessName||"No indicada","RUC",data.metadata.ruc?null:"No indicado","Período",period,"Documentos",invoices.length,`Total ${sheetLabel.toLowerCase()}`,null,null,null,null,null,null]];
  ...
  s.getRange("A2:O2").format={fill:pale,font:{color:ink},rowHeight:27,verticalAlignment:"center"}; s.getRange("A2:I2").format.font={bold:true,color:ink}; s.getRange("J2").format={fill:"#D7EBDD",font:{bold:true,color:green},numberFormat:"#,##0.00"};
  s.getRange("A4:O4").merge(); s.getRange("A4").values=[["DETALLE muestra el primer producto o servicio de cada factura. BASE 0% consolida tarifa 0%, no objeto y exento."]]; s.getRange("A4:O4").format={font:{italic:true,color:muted},rowHeight:22};
  const headers=["RUC","RAZÓN SOCIAL","FACTURA","FECHA","DETALLE","BASE 0%","BASE 15%","BASE 5%","BASE TARIFA ESPECIAL","SUBTOTAL","IVA 15%","IVA 5%","IVA TARIFA ESPECIAL","PROPINA","TOTAL"];
  s.getRange("A6:O6").values=[headers]; styleHeader(s.getRange("A6:O6"));
  const isSales=sheetLabel==="Ventas";
  const rows=invoices.map(({invoice})=>[null,isSales?(invoice.recipientBusinessName||"Consumidor final"):invoice.businessName,null,toDate(invoice.issueDate),invoice.details[0]?.description||"Sin detalle",invoice.vat.base0Consolidated,invoice.vat.base15,invoice.vat.base5,invoice.vat.baseSpecial,invoice.subtotal,invoice.vat.vat15,invoice.vat.vat5,invoice.vat.vatSpecial,invoice.tip,invoice.total]);
  if(rows.length){s.getRange(`A7:O${6+rows.length}`).values=rows;for(let i=0;i<rows.length;i++){const invoice=invoices[i].invoice;s.getRange(`A${7+i}`).formulas=[[textFormula(isSales?invoice.recipientIdentification:invoice.ruc)]];s.getRange(`C${7+i}`).formulas=[[textFormula(invoice.documentNumber)]];}s.getRange(`D7:D${6+rows.length}`).format.numberFormat="dd/mm/yyyy";s.getRange(`F7:O${6+rows.length}`).format.numberFormat="#,##0.00";s.getRange(`A6:O${6+rows.length}`).format.borders={insideHorizontal:{style:"thin",color:line},bottom:{style:"thin",color:line}};const t=s.tables.add(`A6:O${6+rows.length}`,true,`${sheetLabel}Table`);t.style="TableStyleMedium4";t.showBandedRows=true;}
  s.freezePanes.freezeRows(6); widths(s,[16,38,22,13,55,14,14,14,20,14,14,14,20,12,14]);
}
