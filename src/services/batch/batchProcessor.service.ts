...
} from "../validation/invoiceOwnership.service.js";
  ...
  const results = await Promise.all(
    rows.map((row) =>
      limit(async () => {
        ...
        completed += 1;
        console.log(
          `[INFO] ${completed}/${rows.length} ${row.documentNumber}: ${result.status}${result.message ? ` - ${result.message}` : ""}`,
        );
        return result;
      }),
    ),
...
export async function removeDownloadedBatchXml(
  log: BatchLog,
  outputDir: string,
): Promise<void> {
  ...
  const removeIfPresent = async (filePath: string) => {
    try {
      ...
      if ((error as NodeJS.ErrnoException).code !== "ENOENT")
        console.warn(
          `[WARN] No se pudo eliminar el XML temporal ${filePath}: ${errorMessage(error)}`,
        );
    }
  };
  await Promise.all([
    ...log.results.flatMap((item) => [
      removeIfPresent(
        path.join(outputDir, "soap", `${item.accessKey}.authorization.xml`),
      ),
      removeIfPresent(
        path.join(outputDir, "soap", `${item.accessKey}.validity.xml`),
      ),
    ]),
    ...log.results
      .filter(
        (item) =>
          ["DESCARGADO", "XML_CARGADO_MANUAL"].includes(item.status) &&
          item.xmlPath,
      )
      .map(async (item) => {
        await removeIfPresent(item.xmlPath!);
        delete item.xmlPath;
      }),
  ]);
}
...
async function processOne(
  ...
  if (row.validationError)
    return { ...base, status: "ERROR", message: row.validationError };
  ...
  if (await exists(xmlPath)) {
    try {
      ...
      if (invoice.accessKey !== row.accessKey)
        throw new Error("La clave interna no coincide.");
      if (
        ...
        throw new Error(
          "El receptor del XML no coincide con el receptor del TXT.",
        );
      if (options.companyTaxId && options.movement)
        validateInvoiceOwnership(
          invoice,
      ...
      return {
        ...
        status: "ERROR",
        xmlPath,
        message: `XML existente invalido: ${errorMessage(error)}`,
      };
    }
  }
  ...
  try {
    const authorization = await withRetry(
      ...
      (attempt) =>
        console.warn(`[WARN] Reintento ${attempt} para ${row.documentNumber}`),
    );
    await writeFile(
      path.join(soapDir, `${row.accessKey}.authorization.xml`),
      authorization.rawResponse,
      "utf8",
    );
    const authorized = authorization.authorizations.find(
      (item) => item.estado?.trim().toUpperCase() === "AUTORIZADO",
    );
    if (authorized?.comprobante) {
      const parsed = parseInvoiceXml(authorized.comprobante);
      if (parsed.accessKey !== row.accessKey)
        throw new Error(
          "La clave del XML descargado no coincide con la consultada.",
        );
      if (
        !identificationsMatch(
          parsed.recipientIdentification,
          row.recipientIdentification,
        )
      )
        throw new Error(
          "El receptor del XML no coincide con la empresa seleccionada.",
        );
      if (options.companyTaxId && options.movement)
        validateInvoiceOwnership(
          parsed,
      ...
      await writeFile(xmlPath, authorized.comprobante, {
        encoding: "utf8",
        flag: "wx",
      });
      return {
        ...base,
        status: "DESCARGADO",
        ...(authorized.numeroAutorizacion
          ? { authorizationNumber: authorized.numeroAutorizacion }
          : {}),
        ...(authorized.fechaAutorizacion
          ? { authorizationDate: String(authorized.fechaAutorizacion) }
          : {}),
        xmlPath,
      };
    }
    if (authorization.authorizations.length > 0) {
      ...
      return {
        ...
        message: messageFromAuthorization(last),
      };
    }
...
async function classifyWithValidity(
  ...
  await writeFile(
    path.join(soapDir, `${row.accessKey}.validity.xml`),
    validity.rawResponse,
    "utf8",
  );
  const message = validity.messages
    .map((item) => item.informacionAdicional || item.mensaje)
    .filter(Boolean)
    .join(" | ");
  if (/fuera del rango/i.test(message))
    return { ...base, status: "FUERA_DE_RANGO", message };
  if (/no existen datos/i.test(message))
    return { ...base, status: "NO_ENCONTRADO", message };
  const state = validity.authorizationStatus?.toUpperCase();
  if (state === "NO AUTORIZADO")
    return { ...base, status: "NO_AUTORIZADO", message: state };
  if (state === "AUTORIZADO") {
    return {
      ...base,
      status: "PENDIENTE",
      message:
        "El SRI confirma AUTORIZADO, pero el servicio no entrego el XML.",
    };
  }
  return {
    ...base,
    status: "PENDIENTE",
    message: message || validity.queryStatus || "Sin respuesta concluyente.",
  };
}
...
function messageFromAuthorization(
  value: { estado?: string; mensajes?: { mensaje?: unknown } } | undefined,
): string {
